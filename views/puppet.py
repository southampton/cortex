...
def puppet_enc_edit(node):
	...
	""" 
		============= This should somehow be linked to the API ===============
	filename = '/srv/cortex/cortex/views/documentation.json'
	...
	for class_obj in result['puppet_classes']:
		for tag in class_obj['docstring']['tags']:
			if "::" not in class_obj['name'] and class_obj['name'] not in to_be_inserted:
				to_be_inserted.append(class_obj['name'] in outcome)
				to_be_inserted[class_obj['name']] = "init.pp"
				to_be_inserted[class_obj['name']]["init.pp"] = [tag['name']]
				
			elif "::" not in class_obj['name'] and class_obj['name'] in to_be_inserted:
 				to_be_inserted[class_obj['name']]["init.pp"] = [tag['name']]

			elif "::" in class_obj['name'] and class_obj['name'] in to_be_inserted:
				
				to_be_inserted[class_obj['name']][]

			if class_obj['name'] in outcome:
				outcome[class_obj['name']].append(tag['name'])
			else:
				outcome[class_obj['name']] = [tag['name']]
	r = json.dumps(outcome)
	loaded_r = json.loads(r)
	
		=====================   The above code should somehow be linked to the API
	"""
	"""
	...
	for json_file in os.listdir(BASE_JSON_DIR_PATH):
		if json_file.endswith(".json"):
			f = open(BASE_JSON_DIR_PATH+"/"+json_file, 'r')
			content_raw = f.read()
	...
	for module_list in outcome['puppet_classes']:
		for module in module_list:
			app.logger.debug(module.keys())
			if "::" in module['name']:
	...
	curd.connection.autocommit(True)
	"""
