...
def requests():
	...
	# Render
	return render_template('systems/requests.html', statuses=statuses, active='systems', title="Requests", q=q)

################################################################################

@app.route('/systems/requests/json', methods=['POST'])
@cortex.lib.user.login_required
@app.disable_csrf_check
def requests_json():
	"""Used by DataTables to extract information from the systems table in
	...
	# Get results of query
	results = cortex.lib.systems.get_requests(filter_group, search, order_column, order_asc, start, length)
	...
	# Return JSON data in the format DataTables wants
	return jsonify(draw=draw, data=requests_data)
