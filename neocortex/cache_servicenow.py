...
def run(helper, options):
	...
	# parameter)
	r = requests.get('https://' + helper.config['SN_HOST'] + '/cmdb_ci_server.do?JSONv2&sysparm_fields=sys_id,sys_class_name,operational_status,u_number,name,short_description&displayvalue=true', auth=(helper.config['SN_USER'], helper.config['SN_PASS']), headers={'Accept': 'application/json'})
	...
	for row in data['records']:
		...
		curd.execute('INSERT INTO `sncache_cmdb_ci` (`sys_id`, `sys_class_name`, `name`, `operational_status`, `u_number`, `short_description`) VALUES (%s, %s, %s, %s, %s, %s)', (row['sys_id'], row['sys_class_name'], row['name'], row['operational_status'], row['u_number'], row['short_description']))
