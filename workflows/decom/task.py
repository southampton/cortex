...
def action_check_system(action, helper, wfconfig):
	...
	if 'ENTCA_SERVERS' in wfconfig and wfconfig['ENTCA_SERVERS'] is not None:
		...
		if isinstance(entca_servers, str):
		...
		for entca in entca_servers:
			try:
				...
				helper.flash("Warning - An error occured when communicating with Enterprise CA: {ex}".format(ex=ex), "warning")
			else:
	...
	if 'SUDO_LDAP_ENABLE' in wfconfig and wfconfig['SUDO_LDAP_ENABLE']:
		try:
			# Connect to LDAP
			sudo_ldap_conn = ldap3.Connection(
			...
			for domain_suffix in wfconfig['SUDO_LDAP_SEARCH_DOMAINS']:
				...
				if search and sudo_ldap_conn.response:
					for result in sudo_ldap_conn.response:
						distinguished_name = result['dn']

						# Store the sudoHosts for each DN we find
						if distinguished_name not in ldap_dn_data:
							ldap_dn_data[distinguished_name] = {'cn': result['attributes']['cn'][0], 'sudoHost': result['attributes']['sudoHost'], 'action': 'none', 'count': 0, 'remove': []}
						...
						for idx, entry in enumerate(ldap_dn_data[distinguished_name]['sudoHost']):
							if entry.lower() == host.lower():
								ldap_dn_data[distinguished_name]['sudoHost'].pop(idx)
								ldap_dn_data[distinguished_name]['action'] = 'modify'
								ldap_dn_data[distinguished_name]['remove'].append(entry)

			# Determine if any of the DNs are now empty
			for distinguished_name in ldap_dn_data:
				if len(ldap_dn_data[distinguished_name]['sudoHost']) == 0:
					ldap_dn_data[distinguished_name]['action'] = 'delete'

			# Print out system_actions
			for distinguished_name in ldap_dn_data:
				if ldap_dn_data[distinguished_name]['action'] == 'modify':
					for entry in ldap_dn_data[distinguished_name]['remove']:
						system_actions.append({'id': 'sudoldap.update', 'desc': 'Remove sudoHost attribute value ' + entry + ' from ' + ldap_dn_data[distinguished_name]['cn'], 'detail': 'Update object ' + distinguished_name + ' on ' + wfconfig['SUDO_LDAP_URL'], 'data': {'dn': distinguished_name, 'value': entry}})
				elif ldap_dn_data[distinguished_name]['action'] == 'delete':
					system_actions.append({'id': 'sudoldap.delete', 'desc': 'Delete ' + ldap_dn_data[distinguished_name]['cn'] + ' because we\'ve removed its last sudoHost attribute', 'detail': 'Delete ' + distinguished_name + ' on ' + wfconfig['SUDO_LDAP_URL'], 'data': {'dn': distinguished_name, 'value': ldap_dn_data[distinguished_name]['sudoHost']}})
				...
				for suffix in wfconfig['GRAPHITE_DELETE_SUFFIXES']:
					...
					if r.status_code == 200:
						...
						if isinstance(r_json, list) and r_json:
							system_actions.append({'id': 'graphite.delete', 'desc': 'Remove metrics from Graphite / Grafana', 'detail': 'Delete ' + ','.join(r_json) + ' from ' + helper.lib.config['GRAPHITE_URL'], 'data': {'host': host}})
					else:
	...
	if "TENABLE_IO_ENABLE_DECOM" in wfconfig and wfconfig["TENABLE_IO_ENABLE_DECOM"]:
		try:
			if all(k in helper.lib.config and helper.lib.config[k] for k in ["TENABLE_IO_URL", "TENABLE_IO_ACCESS_KEY", "TENABLE_IO_SECRET_KEY"]):
				...
				if r.status_code == 200:
					r_json = r.json()
					for agent in r_json.get("agents", []):
...
def action_sudoldap_update(action, helper, wfconfig):
	try:
		...
		sudo_ldap_conn.modify(action['data']['dn'], {
			'sudoHost': [(ldap3.MODIFY_DELETE, action['data']['value'])]
		})

		return True
	except Exception as ex:
		helper.end_event(success=False, description="Failed to update the object in sudoldap: " + str(ex))
		return False
...
def action_sudoldap_delete(action, helper, wfconfig):
	try:
		...
		# Delete the entry
		sudo_ldap_conn.delete(action['data']['dn'])
...
def action_graphite_delete(action, helper, _wfconfig):
	try:
...
def action_nessus_delete(action, helper, _wfconfig):
	try:
		...
		helper.end_event(success=False, description="Failed to remove the Nessus agent: {ex}".format(ex=ex))
		return False
